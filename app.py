...
st.session_state["select_prompt"] = st.sidebar.selectbox('誰に向けた文章ですか?', ['友人', '先生', '上司', '論文など'])
st.session_state["answer"] = st.sidebar.slider('校正結果の出力数を決めてください。', 1, 3, 1) 
...
if "select_prompt" == '友人' and "messages" not in st.session_state:
    st.session_state["messages"] = [
        ...
        {"role": "user", "content": "校正結果は" + str("answer") + "個出力してください。"},
        ]
elif "select_prompt" == '先生' and "messages" in st.session_state:
    st.session_state["messages"] = [
        {"role": "system", "content": "あなたは優秀な文章校正アシスタントAIです。"},
        {"role": "user", "content": "先生へ宛てたメッセージを校正してください。尚、『敬語を使う』『誤字・脱字の訂正』『曖昧な表現の訂正』『要点の強調』という4つの条件を遵守してください。"},
        {"role": "user", "content": "校正結果は" + str("answer") + "個出力してください。"},
        ]
elif "select_prompt" == '上司' and "messages" in st.session_state:
    st.session_state["messages"] = [
        {"role": "system", "content": "あなたは優秀な文章校正アシスタントAIです。"},
        {"role": "user", "content": "上司へ宛てたメッセージを校正してください。尚、『礼節を弁えた,簡潔な文章にする』『誤字・脱字の訂正』『曖昧な表現の訂正』『要点の強調』という4つの条件を遵守してください。"},
        {"role": "user", "content": "校正結果は" + str("answer") + "個出力してください。"},
        ]
elif "select_prompt" == '論文など' and "messages" in st.session_state:
    st.session_state["messages"] = [
        {"role": "system", "content": "あなたは優秀な文章校正アシスタントAIです。"},
